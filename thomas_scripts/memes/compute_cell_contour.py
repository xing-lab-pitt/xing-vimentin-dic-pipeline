...
contour_points_and_obj = find_contour_points(
    sample_path, sample_img_list, contour_value=0.5)
...
mean_contour, iters = align_contours(
    cell_contours, allow_reflection=True, allow_scaling=False, max_iters=20)
...
for i in range(len(cell_contours)):
    scale_back = utility_tools.decompose_homogenous_transform(
        cell_contours[i].to_world_transform)[1]
...
pca_contours = contour_class.PCAContour.from_contours(
    ...
    required_variance_explained=0.98,
...
for pci in range(pca_contours.position.shape[0]):
    ...
    print(mode_std_1)
    print(mode_std_2)
    shape_array0 = contour_class.PCAContour.points_at_position(
        pca_contours, cell_posi)
    shape_array1 = contour_class.PCAContour.points_at_position(
        pca_contours, mode_std1)
    shape_array2 = contour_class.PCAContour.points_at_position(
        pca_contours, mode_std2)
    shape_array3 = contour_class.PCAContour.points_at_position(
        pca_contours, mode_std_1)
    shape_array4 = contour_class.PCAContour.points_at_position(
        pca_contours, mode_std_2)
...
all_cord = []
for i in range(len(cell_contours)):
    cell_posi = contour_class.PCAContour.find_position(
        pca_contours, cell_contours[i])
