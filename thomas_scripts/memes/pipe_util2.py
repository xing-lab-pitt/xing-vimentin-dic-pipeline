...
def folder_img_show(folder, num, gray = 1):
    ...
    while i<num:
        ...
        if gray:
            plt.imshow(imread(img_path_list[i])/256., cmap=gray )
        else:
            plt.imshow(imread(img_path_list[i])/256., cmap='gray' )
        i = i+1
...
def folder_img_edge_show(folder, num, gray = 0):
    ...
    while i<num:
        fig.add_subplot(rows,5, i+1)
        if gray:
            plt.imshow( filters.roberts(imread(img_path_list[i]))/256., cmap = gray)
        else:
            plt.imshow(filters.roberts(imread(img_path_list[i]))/256., cmap="gray")
        i = i+1
...
def list_label_show(label_list, fig_num, w_num, h_num, 
                    costom_cmap = "coolwarm",w = 20, h = 20):
    """
