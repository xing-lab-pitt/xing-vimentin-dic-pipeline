...
def decompose_homogenous_transform(transform):
    ...
    T = transform[:2, :2]
    # decompose transform T into rotation/reflection Q and scale/shear S via
    # polar decomposition
...
def b_spline_to_bezier_series(tck, per=False):
    ...
    new_tck = tck
    if per:
        # ignore the leading and trailing k knots that exist to enforce
        # periodicity
    ...
    for x in knots_to_consider:
        ...
        if remainder != 0:
            # add enough knots to bring the current multiplicity up to the
