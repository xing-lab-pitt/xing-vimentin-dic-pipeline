...
def prep_train_data_list(imgs, gts):
    ...
    for i in range(len(imgs)):
        ...
        img_zoom, gt_zoom = random_zoom(
            np.expand_dims(
                img, axis=2), np.expand_dims(
                gt, axis=2), zoom_range=(
                0.5, 1))
    ...
    if random_eah:
        ...
        if eah_flag == 1:
            ...
            img = equalize_adapthist(
                img, clip_limit=clip_value, nbins=hist_bins)
...
def keep_aspect_resize(img, obj_h, obj_w, fill_mode='nearest'):
    ...
    if fill_mode == 'nearest':
        inds = ndimage.distance_transform_edt(
            img == 0, return_distances=False, return_indices=True)
        img = img[tuple(inds)]
    if fill_mode == 'median':
...
def prep_icnn_seg_train_data(img_path, obj_h, obj_w, classes = ['single_cell', 'cell_fragment', 'multi_cells']):
    ...
    for cla in classes:
        img_sub_path = img_path + '/' + cla + "img"
        label_sub_path = img_path + "/" + cla + "label"
        img_list = sorted(listdir(img_sub_path))
        label_list = sorted(listdir(label_sub_path))
        i = 0
        while (i < len(img_list)):

            img = imread(img_sub_path + '/' + img_list[i])
            mask = imread(label_sub_path + '/' + label_list[i])
...
def prep_icnn_am_train_data(img_path, obj_h, obj_w):
    ...
    for cla in classes:
        ...
        while i < len(img_list):
            ...
            img_nbg = keep_aspect_resize(
                img, obj_h, obj_w, fill_mode='nearest')
            img_mbg = keep_aspect_resize(img, obj_h, obj_w, fill_mode='median')
            img_lbg = keep_aspect_resize(
                img, obj_h, obj_w, fill_mode='random_low')
            img_hbg = keep_aspect_resize(
                img, obj_h, obj_w, fill_mode='random_high')
            ...
            img_blur_mbg = obj_transform(
                gaussian(
                    ...
                    sigma=0.5,
                    preserve_range=True))
            img_blur_lbg = obj_transform(
                gaussian(
                    img_lbg,
                    sigma=0.5,
                    preserve_range=True))
            img_blur_hbg = obj_transform(
                gaussian(
                    img_hbg,
                    sigma=0.5,
                    preserve_range=True))
            ...
            img_unsharp_nbg = obj_transform(
                unsharp_mask(img_nbg, preserve_range=True))
            img_unsharp_mbg = obj_transform(
                unsharp_mask(img_mbg, preserve_range=True))
            img_unsharp_lbg = obj_transform(
                unsharp_mask(img_lbg, preserve_range=True))
            img_unsharp_hbg = obj_transform(
                unsharp_mask(img_hbg, preserve_range=True))
...
def generate_single_cell_img_edt(
        img_path,
        seg_path,
        img_list,
        seg_img_list,
    ...
    crop_img = single_cell_img[candi_box[0]
        :candi_box[2], candi_box[1]:candi_box[3]]
...
def generate_single_cell_img_edt_test(
        img_path,
        seg_path,
        img_list,
        seg_img_list,
