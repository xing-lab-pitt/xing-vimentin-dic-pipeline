...
def compute_overlap_matrix(img_path, img_list, img_num_1, img_num_2):
    ...
    """
    img_path - /seg path, where pictures are labels.
    img_list - name string list for all imgs in the /seg folder
    img_num_1 - int
    img_num_2 - int

    This return a matrix for adjacent frame, and listing pixel number of overlapping between every pair of single cell in the frame.
    """
    frame_1 = imread(img_path + "/" + img_list[img_num_1 - 1])
    frame_2 = imread(img_path + "/" + img_list[img_num_2 - 1])
    nb_cell_1 = np.max(frame_1)  # number of labels in frame 1
    nb_cell_2 = np.max(frame_2)  # number of labels in frame 2
...
def judge_mol_type(frame_overlap, source_obj, target_obj):  # obj max_overlap relation judge
    rel_flag = 0
    """
    source_obj - is a number, for rows in the frame_overlap. 
    """
...
def am_obj_info(am_record, df):
    """
    For each object (every row) in am_record.csv, find it's area and center.
    :param am_record:
    :param df:
    :return:
    """
    ...
    df = df[
        ["ImageNumber", "ObjectNumber", "Cell_AreaShape_Center_X", "Cell_AreaShape_Center_Y", "Cell_AreaShape_Area"]
    ].copy()

    am_df_merge = am_record.merge(right=df, how="left", on=["ImageNumber", "ObjectNumber"], copy=False)
    am_arr = am_df_merge[["ImageNumber", "ObjectNumber"]].values
    am_xy = am_df_merge[["Cell_AreaShape_Center_X", "Cell_AreaShape_Center_Y"]].values
    am_area = am_df_merge["Cell_AreaShape_Area"].values
...
def find_am_sisters(F, mitosis_max_distance=50, size_simi_thres=0.7):
    ...
    size_simi_thres - between 0 and 1, the similarity over area (odd measure)
    """
    ...
    Fi_list = []
    Fj_list = []
    frames = np.unique(F[:, IIDX])

    for f in frames:
        F_sub = F[F[:, IIDX] == f]

        # Find all daughter pairs within same frame
        i, j = np.where(F_sub[:, np.newaxis, IIDX] == F_sub[np.newaxis, :, IIDX])  # all pairs
        i, j = i[i < j], j[i < j]  # get rid of duplicates and self-compares (upper triangle)

        dmax = max_distance * 2 - np.sqrt(np.sum((F_sub[i, :2] - F_sub[j, :2]) ** 2, 1))  # dis for all pairs
        dist_mask = dmax >= 0
        i, j, dmax = i[dist_mask], j[dist_mask], dmax[dist_mask]  # remove where dist_mask smaller than 0
        size_simi = 1 - abs((F_sub[i, AIDX] - F_sub[j, AIDX]) / (F_sub[i, AIDX] + F_sub[j, AIDX]))
        size_mask = np.zeros(i.shape, dtype=bool)

        ti = f  # current img_num
        ii = np.arange(len(i))  # get the index for all paired object index.

        match_inds = []
        mask_ti = F_sub[i, IIDX] == ti  # get an bool list that true only for this img_num
        ti_size_simi = size_simi[ii[mask_ti]]  # get the size_simi only for this img_num
        sort_inds = np.argsort(-ti_size_simi)  # sorting size_simi, decending
        for si in sort_inds:  # for each size_simi for the obj

            if ti_size_simi[si] < size_simi_thres:  # if current size_simi smaller than threshold, go to next size_simi
                break
        ...
        Fi_list.append(F_sub[i, 2:4])
        Fj_list.append(F_sub[j, 2:4])

    Fi = np.concatenate(Fi_list, axis=0)
    Fj = np.concatenate(Fj_list, axis=0)
    # print(Fi.shape, Fj.shape)
    # -----------------------------
    # hj_util.print_time()
    return np.column_stack((Fi, Fj))
