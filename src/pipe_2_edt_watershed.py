...
def generate_single_cell_img_env(img, rps, obj_h, obj_w, obj_num):
    ...
    crop_img = obj_transform(
        keep_aspect_resize(crop_img, obj_h, obj_w), random_eah=False
    )  # obj_transform, normalize picture, and then add an axis at the end.
    crop_img = np.expand_dims(crop_img, axis=0)
...
def simple_edt_watershed(
    img_path, output_path, icnn_am_weights, icnn_seg_weights, small_obj_thres=20, obj_h=128, obj_w=128
):
    ...
    am_record.to_csv(dir_path + "am_record.csv", index=False, encoding="utf-8")
