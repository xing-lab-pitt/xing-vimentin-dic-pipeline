...
def traj_reconganize2(output_path):
    ...
    if n_mitoses > 0:
        ...
        for i_n, o_n in mitoses[:, :2]:
            obj_am_flag = 0
            if ((am_record["ImageNumber"] == i_n) & (am_record["ObjectNumber"] == o_n)).any():
                obj_am_flag = np.asscalar(
                    am_record.loc[
                        (am_record["ImageNumber"] == i_n) & (am_record["ObjectNumber"] == o_n), "am_flag"
                    ].values
                )
            mother_am_flag.append(obj_am_flag)

            cur_traj_label = np.asscalar(
                df.loc[(df["ImageNumber"] == i_n) & (df["ObjectNumber"] == o_n)]["Cell_TrackObjects_Label"].values
            )
            traj_len = len(df.loc[df["Cell_TrackObjects_Label"] == cur_traj_label, "Cell_AreaShape_Area"].values)
        ...
        mother_am_flag, sis1_am_flag, sis2_am_flag = (
            np.array(mother_am_flag),
            np.array(sis1_am_flag),
            np.array(sis2_am_flag),
        )
        mother_traj_len, sis1_traj_len, sis2_traj_len = (
            np.array(mother_traj_len),
            np.array(sis1_traj_len),
            np.array(sis2_traj_len),
        )
        ...
        for sis1_i_n, sis1_o_n, sis2_i_n, sis2_o_n in mitoses[:, 2:]:
            size_corr = cal_size_correlation(
                df, sis1_i_n, sis1_o_n, sis2_o_n, high_traj_len_thres=4, low_traj_len_thres=3
            )
        ...
        for i in range(mitoses.shape[0]):
            ...
            if (
                ...
                mitosis_labels.append(
                    np.asscalar(
                        df.loc[
                            (df["ImageNumber"] == mitoses[i][0]) & (df["ObjectNumber"] == mitoses[i][1]),
                ...
                mitosis_labels.append(
                    np.asscalar(
                        df.loc[
                            (df["ImageNumber"] == mitoses[i][2]) & (df["ObjectNumber"] == mitoses[i][3]),
                            "Cell_TrackObjects_Label",
                        ].values
                    )
                )
                ...
                mitosis_labels.append(
                    np.asscalar(
                        df.loc[
                            (df["ImageNumber"] == mitoses[i][4]) & (df["ObjectNumber"] == mitoses[i][5]),
                            "Cell_TrackObjects_Label",
                        ].values
                    )
                )
    ...
    for i in range(mitoses.shape[0]):
        ...
        mother_traj_label = np.asscalar(
            df.loc[
                (df["ImageNumber"] == mother_i_n) & (df["ObjectNumber"] == mother_o_n), "Cell_TrackObjects_Label"
            ].values
        )
        sis1_i_n = mitoses[i][2]
        sis1_o_n = mitoses[i][3]
        sis1_traj_label = np.asscalar(
            df.loc[(df["ImageNumber"] == sis1_i_n) & (df["ObjectNumber"] == sis1_o_n), "Cell_TrackObjects_Label"].values
        )
        sis2_i_n = mitoses[i][4]
        sis2_o_n = mitoses[i][5]
        sis2_traj_label = np.asscalar(
            df.loc[(df["ImageNumber"] == sis2_i_n) & (df["ObjectNumber"] == sis2_o_n), "Cell_TrackObjects_Label"].values
        )
    ...
    mitoses = mitosis_record[["mother_i_n", "mother_o_n", "sis1_i_n", "sis1_o_n", "sis2_i_n", "sis2_o_n"]].values
    mitosis_labels = (
        mitosis_record[["mother_traj_label", "sis1_traj_label", "sis2_traj_label"]].values.flatten().tolist()
    )
    mother_cells = mitosis_record[["mother_i_n", "mother_o_n"]].values.tolist()

    daughter_cells = []
    daughter_cells.extend(mitosis_record[["sis1_i_n", "sis1_o_n"]].values.tolist())
    daughter_cells.extend(mitosis_record[["sis2_i_n", "sis2_o_n"]].values.tolist())
    print(mitoses.shape)
    ...
    for i in range(len(false_mitosis_obj)):
        ...
        fm_label = np.asscalar(
            df.loc[(df["ImageNumber"] == fm_i_n) & (df["ObjectNumber"] == fm_o_n), "Cell_TrackObjects_Label"].values
        )
    ...
    # relabel df and save
    df = relabel_traj(df)
    df.to_csv(dir_path + "Per_Object_mitosis.csv", index=False, encoding="utf-8")
