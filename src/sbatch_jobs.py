...
python /net/capricorn/home/xing/huijing/Segmentation/scripts/Image_analysis_pipeline_vim_dic/train_icnn_am.py %s %s
    """%(ofile_name, train_folder, weight)
    s = open(sfile_name, "w")
    s.write(contents)
    s.close()
...
def sbatch_file_1_edt(output_folder, 
                      edt_input_path, edt_output_path, 
                      edt_weight_file, edt_model_mode,
                      name = None ):
    """
    output_folder = string, folder for storing the .o file. Usually the same as script folder
    edt_input_path
...
python /net/capricorn/home/xing/huijing/Segmentation/scripts/Image_analysis_pipeline_vim_dic/pipe_1_img_edt.py %s %s %s %s
    """%(ofile_name, edt_input_path, edt_output_path, edt_weight_file, edt_model_mode)
    s = open(sfile_name, "w")
    s.write(contents)
    s.close()
...
def sbatch_file_2_watershed(script_folder, 
                            watershed_img_path, watershed_output_path, 
...
python /net/capricorn/home/xing/huijing/Segmentation/scripts/Image_analysis_pipeline_vim_dic/pipe_2_edt_watershed.py %s %s %s %s
    """%(ofile_name, watershed_img_path, watershed_output_path, icnn_am_weights, icnn_seg_weights)
    s = open(sfile_name, "w")
    s.write(contents)
    s.close()
...
def sbatch_file_3_cellprofiler(script_folder, 
                               cp_output_path,
                               name = None):
    now = datetime.now()
    dt_string = now.strftime("%d-%m-%Y-%H%M")
...
def sbatch_job_1_edt(output_folder, 
                     edt_input_path, edt_output_path, 
                     edt_weight_file, edt_model_mode,
                     name = None):
    """Generating the sbatch code and then submit the job."""
    scr_path = sbatch_file_1_edt(
        output_folder, 
        edt_input_path, edt_output_path, 
        edt_weight_file, edt_model_mode,
        name)
...
def sbatch_job_2_watershed(script_folder, 
                           watershed_img_path, watershed_output_path, 
                           icnn_am_weights, icnn_seg_weights,
                           name = None):
...
def sbatch_job_3_cellprofiler(script_folder, 
                              cp_output_path,
                              name = None):
    ...
    scr_path = sbatch_file_3_cellprofiler(
        script_folder, 
        cp_output_path,
        name)
    cmd = "sbatch %s"%scr_path
