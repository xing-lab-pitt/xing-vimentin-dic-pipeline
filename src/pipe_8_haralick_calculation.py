...
def single_folder_run(img_folder, output_path):
    ...
    vim_img_list = sorted(listdir(vim_img_path))
...
#         print(img_num,mean_intensity,std_intensity)
        for obj_num in np.arange(1, np.amax(cell_seg) + 1):
            ...
            vim_features = [mean_intensity[obj_num - 1], 
            ...
            cells[ind].set_fluor_features(
                'vimentin', feature_list, vim_features)
...
for posi in range(posi_start, posi_end + 1):
    p = Process(target=single_posi_run, args=(posi,))
    p.start()
    processes.append(p)
