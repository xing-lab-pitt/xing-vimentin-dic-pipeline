...
sys.path.insert(1, "/net/capricorn/home/xing/tch42/Projects/a549_pcna/src/3-compute/tools")
...
for ti in np.arange(len(img_str)):
    ...
    img, removed_cols, removed_rows = bbr(
        img
    )  # sometimes the images captured by the nikon microscope has black borders
    #     bib = np.delete(bib,removed_rows,axis=0); bib = np.delete(bib,removed_cols,axis=1)
    boundary = np.delete(boundary, removed_rows, axis=0)
    boundary = np.delete(boundary, removed_cols, axis=1)
    bwdist = np.delete(bwdist, removed_rows, axis=0)
    bwdist = np.delete(bwdist, removed_cols, axis=1)
    interior = np.delete(interior, removed_rows, axis=0)
    interior = np.delete(interior, removed_cols, axis=1)
