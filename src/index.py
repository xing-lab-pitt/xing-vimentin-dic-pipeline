...
class Indexes(object):
    ...
    def __init__(self, counts):
        ...
        if len(non_empty_indices) > 0:
            self.__rev_idx[self.__fwd_idx[non_empty_indices[0]]
                           ] = non_empty_indices[0]
            if len(non_empty_indices) > 1:
                distance_to_next = non_empty_indices[1:] - \
                    non_empty_indices[:-1]
                self.__rev_idx[self.__fwd_idx[non_empty_indices[1:]]
                               ] = distance_to_next
            ...
            for i, count in enumerate(counts[:-1]):
                ...
                self.__idx.append(
                    (indexes / modulos[self.__rev_idx]).astype(int))
