...
def cal_border(img_num, seg_img, border_flag):
    ...
    for region, neighbors_per_region in zip(regions, neighbors_per_regions):
        if region != 0:
            ...
            unique_neighbors, neighbor_border_len = npi.count(neighbors_per_region)
            border_ratio = neighbor_border_len.astype(np.float) / neighbor_border_len.sum()
            ...
            non0_inds = [i for i, uni_nei in enumerate(sort_unique_neighbors) if uni_nei != 0]

            nonfree_border = sum(sort_border_ratio[non0_inds])

            neighbor_list = [
                (uni_nei, nei_border_len)
                for uni_nei, nei_border_len in zip(
                    sort_unique_neighbors[non0_inds].tolist(), sort_neighbor_border_len[non0_inds].tolist()
                )
            ]

            neighbor_info.append(
                [img_num, region.astype(int), border_flag[(region - 1).astype(int)], nonfree_border, neighbor_list]
            )
...
def local_neighbor_info_cal(img_path, output_path):
    ...
    df = pd.read_csv(output_path + str(posi) + "/Per_Object_relink_" + str(posi) + ".csv")
    ...
    for i in range(len(ori_img_list)):
        ...
        m, n = ndimage.distance_transform_edt(seg_img == 0, return_distances=False, return_indices=True)
        ...
        for cell_label in range(1, np.amax(dt_seg_img) + 1):
            ...
            if np.amax(label_img) > 1:
                ...
                single_dt_obj_img = ndimage.binary_propagation(single_obj_img, mask=label_img)
