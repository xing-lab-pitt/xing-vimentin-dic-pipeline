...
class PointSet(object):
    ...
    def alignment_angle(self):
        ...
        original alignment."""
        rotate_reflect = utility_tools.decompose_homogenous_transform(
            self.to_world_transform)[0]
    ...
    def transform(self, transform):
        ...
        self.points = utility_tools.homogenous_transform_points(
            self.points, transform)

    def translate(self, translation):
        """Translate the points by the given [x,y] translation."""
        self.transform(
            utility_tools.make_homogenous_transform(
    ...
    def descale(self):
        ...
        rotate_reflect, scale_shear, translation = utility_tools.decompose_homogenous_transform(
            self.to_world_transform)
        transform = utility_tools.make_homogenous_transform(
    ...
    def rotate(self, rotation, in_radians=True):
        ...
        self.transform(utility_tools.make_homogenous_transform(
            transform=[[c, s], [-s, c]]))
    ...
    def to_file(self, filename):
        ...
        try:
            ...
            raise IOError(
                'Could not open file "%s" for saving. (Error: %s)' %
                (filename, e))
    ...
    def axis_align(self):
        ...
        # If we're rotating by more than pi/2 radians, just go the other
        # direction.
...
class Contour(PointSet):
    ...
    def spline_derivatives(self, begin, end, derivatives=1):
        ...
        ret = [numpy.transpose(fitpack.splev(points, tck, der=d))
    ...
    def normalized_curvature(self, begin=None, end=None):
        ...
        the value is higher."""
        return numpy.absolute(self.curvatures(begin, end)
                              ).mean() * self.length(begin, end)
    ...
    def inward_normals(self, positions=None):
        ...
        if self.signed_area() > 0:
            inward_normals *= -1
        inward_normals /= numpy.sqrt((inward_normals **
...
#      self.points[-1] = self.points[0]
    ...
    def _make_acyclic(self):
        ...
        if numpy.alltrue(self.points[-1] == self.points[0]):
            self.points = numpy.resize(
                self.points, [
                    self.points.shape[0] - 1, self.points.shape[1]])
    ...
    def to_spline(self, smoothing=0, spacing_corrected=False):
        ...
        points[-1] = points[0]
        tck, uout = fitpack.splprep(
            x=points.transpose(), u=u, per=True, s=smoothing)
        return tck, uout
    ...
    def to_bezier(self, match_curves_to_points=False, smooth=True):
        ...
        if match_curves_to_points:
            # to_insert = numpy.setdiff1d(u,
            # numpy_compat.unique(tck[0]))#-------note wwk------the
            # numpy_compat.py are removed
    ...
    def resample(
        ...
        while (iters < max_iters and ms_change > min_ms_change):
            ...
            arc_spans = (roll(positions, -1, axis=0) -
                         roll(positions, 1, axis=0)) % (l + 1)
    ...
    def global_reorder_points(self, reference):
        ...
        while(True):
            ...
            dp = self.as_offset_points(
                best_offset + 1).rms_distance_from(reference)
            dn = self.as_offset_points(
                best_offset - 1).rms_distance_from(reference)
            if d < dp and d < dn:
                break
            elif dp < dn:
    ...
    def _local_point_ordering_search(
            self,
            reference,
            distance_function,
            max_iters=None):
    ...
    def local_reorder_points(self, reference, max_iters=None):
        ...
        """
        return self._local_point_ordering_search(
            reference, self.__class__.rms_distance_from, max_iters)
    ...
    def local_best_alignment(
        ...
        def find_distance(contour, reference):
            return pdf(
                ...
                allow_translation)
        return self._local_point_ordering_search(
            reference, find_distance, max_iters)
    ...
    def global_best_alignment(
        ...
        return self.local_best_alignment(
            reference,
            weights,
            allow_reflection,
            allow_scaling,
    ...
    def find_shape_intersections(self, ray_starts, ray_ends):
        ...
        for start, end in zip(ray_starts, ray_ends):
            radii, positions = utility_tools.line_intersections(
                start, end, s0, s1)
            intersects_in_segment = (positions <= 1) & (positions >= 0)
            intersect_radii = radii[intersects_in_segment]
            intersect_positions = (
                all_points + positions)[intersects_in_segment]
        ...
        return numpy.array(
            intersections, dtype=float), numpy.array(
            point_numbers, dtype=float)
    ...
    def find_nearest_point(self, point):
        ...
        s0 = self.points
        s1 = numpy.roll(s0, -1, axis=0)
        closest_points, positions = utility_tools.closest_point_to_lines(
            point, s0, s1)
        positions.clip(0, 1)
        positions += numpy.arange(len(self.points))
        square_distances = (
            (point[:, numpy.newaxis] - closest_points)**2).sum(axis=1)
...
class ContourAndLandmarks(Contour):
    ...
    def set_weights(self, landmark_weights):
        ...
        if total_landmark_weight > 1:
            raise ValueError(
                'The total weight assigned to the landmarks must not be greater than one.')
    ...
    def rms_distance_from(self, reference):
        ...
        return numpy.sqrt(((self.weights[:, numpy.newaxis] *
                            (self._get_points_and_landmarks() -
                             reference._get_points_and_landmarks()))**2).mean())
...
class PCAContour(Contour):
    ...
    def from_contours(
            cls,
            contours,
            required_variance_explained=0.95,
        ...
        if not utility_tools.all_same_shape(data):
            raise ValueError(
                'All contours must have the same number of points in order to perform PCA.')
        units = [c.units for c in contours]
        if not numpy.alltrue([u == units[0] for u in units]):
            raise ValueError(
                'All contours must have the same units in order to produce a PCA shape model from them.')
        units = units[0]
        scales = [utility_tools.decompose_homogenous_transform(c.to_world_transform)[
            1] for c in contours]
        ...
        vals = pca.pca_dimensionality_reduce(
            numpy.array(
                data,
                dtype=numpy.float32),
            required_variance_explained)
    ...
    def transform(self, transform):
        ...
        """
        Contour.transform(self, transform)
        self.mean = utility_tools.homogenous_transform_points(
            self.mean, transform)
        ...
        scale_rotate[2, :2] = 0
        self.modes = numpy.array([utility_tools.homogenous_transform_points(
            mode, scale_rotate) for mode in self.modes])
...
class CentralAxisContour(Contour):
    ...
    _instance_data.update(
        {
            'axis_positions': numpy.zeros(
                (0,)), 'central_axis': numpy.zeros(
                (0, 2)), 'top_points': numpy.zeros(
                    (0, 2)), 'bottom_points': numpy.zeros(
                        (0, 2))})
    ...
    def from_contour(
        ...
        try:
            ...
            scale_steps = numpy.linspace(
                7, num_points, scale_steps, endpoint=True, dtype=int)
        tck, uout = contour.to_spline()
        start_pos, end_pos = numpy.transpose(fitpack.splev([start, end], tck))
        contour = cls(other=contour, central_axis=numpy.array(
            [start_pos, end_pos]))
        ...
        for points in scale_steps:
            ...
            new_axis = numpy.transpose(fitpack.splev(
                numpy.linspace(0, uout[-1], points, endpoint=True), tck))
    ...
    def reverse_central_axis(self):
        self.axis_positions = numpy.roll(
            self.axis_positions, len(
                self.axis_positions) // 2)
        self.central_axis = self.central_axis[::-1]
        self.top_points, self.bottom_points = self.bottom_points[::-
                                                                 1], self.top_points[::-1]
    ...
    def estimate_axis_positions(self, axis, endpoints=None):
        ...
        intersect_points, arc_pos = self.find_shape_intersections(
            axis[1:-1], axis[1:-1] + normals)
        arc_pos = sorted(numpy.concatenate([[start, end], arc_pos.flatten()]))
        self.axis_positions = numpy.roll(
            arc_pos, -numpy.searchsorted(arc_pos, start))
        # if start < end:
    ...
    def subdivide_axis(self):
        ...
        normals[:, 1] = -d1[:, 0]
        intersect_points, arc_pos = self.find_shape_intersections(
            midpoints, midpoints + normals)
        mid_intersection = intersect_points.sum(axis=1) / 2.0
        new_midpoints = midpoints + \
            mid_intersection[:, numpy.newaxis] * normals
        new_axis = numpy.empty(
            (len(
                self.central_axis) +
                len(new_midpoints),
                2),
    ...
    def center_and_space_axis(
        ...
        # reverse bottom forces to match up in order with the top of each line
        # pair
        ...
        while (iters < max_iters and ms_change > min_ms_change):
            ...
            torsion[:, 0] = 2 * (axis_der * TmB).sum(axis=1) * \
                TmB[:, 1] / torsion_denom
            torsion[:, 1] = 2 * (-axis_der * TmB).sum(axis=1) * \
                TmB[:, 0] / torsion_denom
            ...
            endpoint_der = endpoint_step * 2 * \
                (endpoint_vec * TmBdotEnd * TmBendSq - TmBend * TmBdotEnd**2) / denom
            ...
            curvatures = curvature_step * 2 * \
                (minus_previous[1:] - minus_previous[:-1])
            curvature_vals = (curvatures[:-2] -
                              2 * curvatures[1:-1] + curvatures[2:])
            ...
            overlap_derivatives = exp(-distances /
                                      spacing_scale) * point_minus_prev / distances
            overlap = overlap_step * \
                (roll(overlap_derivatives, -1, axis=0) - overlap_derivatives)
            ...
            axis_positions %= l
            # find the point midway between the first pair of points along the
            # axis
            (c1, c2), (d1, d2) = self.find_contour_midpoints(
                axis_positions[1], axis_positions[-1])
            if abs(d1) < abs(d2):
                start = axis_positions[start_i] = c1
            else:
                start = axis_positions[start_i] = c2
            (c1, c2), (d1, d2) = self.find_contour_midpoints(
                axis_positions[n_pairs], axis_positions[n_pairs + 2])
            if abs(d1) < abs(d2):
            ...
            axis_positions = roll(axis_positions, -
                                  searchsorted(axis_positions, start))
    ...
    def axis_cumulative_distances(self):
        return numpy.add.accumulate(numpy.concatenate(
            [[0], self.axis_interpoint_distances()]))
    ...
    def axis_interpoint_distances(self):
        return utility_tools.norm(
            self.central_axis[1:] - self.central_axis[:-1], axis=0)
    ...
    def axis_baseline_distances(self):
        ...
        the other."""
        return utility_tools.signed_distances_to_line(
    ...
    def axis_extrema(self, min_distance=0):
        ...
        minima = utility_tools.local_maxima(-distances,
        ...
        return numpy.sort(numpy.concatenate(
            [[0, len(self.central_axis) - 1], indices]))
    ...
    def axis_wavelength(self, min_distance=0):
        ...
        points, positions = utility_tools.closest_points_to_line(
            self.central_axis, self.central_axis[0], self.central_axis[-1])
        total_len = numpy.sqrt(
            ((self.central_axis[0] - self.central_axis[-1])**2).sum())
        positions *= total_len
        extrema_positions = positions[self.axis_extrema(min_distance)]
        inter_extrema_distances = extrema_positions[1:] - \
            extrema_positions[:-1]
    ...
    def axis_diameters(self, begin=None, end=None):
        ...
        distances = numpy.sqrt(
            ((self.top_points - self.bottom_points)**2).sum(axis=1))
    ...
    def axis_normalized_curvature(self, begin=None, end=None):
        ...
        return numpy.absolute(self.axis_curvatures(
            begin, end)).mean() * self.axis_length(begin, end)

    def axis_normals(self):
        perpendiculars = numpy.empty(self.central_axis.shape, dtype=float)
        perpendiculars[[0, -
                        1]] = utility_tools.find_perp(self.central_axis[[0, -
                                                                         2]], self.central_axis[[1, -
                                                                                                 1]])
        bisectors = utility_tools.find_bisector(
            self.central_axis[:-2], self.central_axis[1:-1], self.central_axis[2:])
        perps = utility_tools.find_perp(
            self.central_axis[:-2], self.central_axis[2:])
    ...
    def axis_to_spline(
        ...
        if end_weight is not None:
            ...
            weights = None
        tck, uout = fitpack.splprep(
            x=self.central_axis.transpose(), u=u, s=smoothing, w=weights)
        return tck, uout
    ...
    def axis_top_bottom_to_spline(self):
        ...
        bottom = numpy.concatenate(
            [axis_positions[axis_points - 1:], [start + contour_points]])[::-1]
    ...
    def resample_axis(self, num_points):
        ...
        positions = numpy.linspace(
            0, axis_points - 1, num_points, endpoint=True)
    ...
    def _to_gnuplot(self):
        import Gnuplot
        ribs = Gnuplot.Data(
            zip(self.top_points, self.bottom_points), with_='lines lt 1')
...
def calculate_mean_contour(contours):
    ...
    if not utility_tools.all_same_shape(all_points):
        raise ContourError(
            'Cannot calculate mean of contours with different numbers of points.')
    mean_points = numpy.mean(all_points, axis=0)
    units = [c.units for c in contours]
    if not numpy.alltrue([u == units[0] for u in units]):
        raise ContourError(
            'All contours must have the same units in order calculate their mean.')
    ...
    if numpy.alltrue([isinstance(c, ContourAndLandmarks) for c in contours]):
        ...
        if not utility_tools.all_same_shape(all_landmarks):
            raise ContourError(
                'Cannot calculate mean of contours with different numbers of landmarks.')
...
def from_file(filename, force_class=None):
    ...
    try:
        ...
        if original_class is not None:
            raise IOError(
                'Could not load file "%s" as a %s. (Error: %s)' %
                (filename, original_class.__name__, e))
        else:
            raise IOError(
                'Could not load file "%s" as any kind of Contour. (Error: %s)' %
                (filename, e))
...
def _compatibility_filter_data(data, desired_class=None):
    if 'cls' not in data:
        ...
        if 'pcs' in data:
            ...
            raise ContourError(
                "Cannot determine type of old-style contour file!")
    else:
        return data
