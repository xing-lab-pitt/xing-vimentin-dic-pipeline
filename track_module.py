...
def am_obj_info(am_record, df):
    ...
    df = df[['ImageNumber','ObjectNumber','Cell_AreaShape_Center_X', 'Cell_AreaShape_Center_Y', 'Cell_AreaShape_Area']].copy()

    am_df_merge = am_record.merge(
        right = df,
        how = "left",
...
def find_am_sisters(F, mitosis_max_distance=50, size_simi_thres=0.7):
    ...
    for f in frames:
        ...
        for si in sort_inds:  # for each size_simi for the obj
            ...
            if ti_size_simi[
                si] < size_simi_thres:  # if current size_simi smaller than threshold, go to next size_simi
                break
