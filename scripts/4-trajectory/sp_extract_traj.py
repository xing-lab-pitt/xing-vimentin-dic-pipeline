...
sct_path = output_path + "single_cell_traj_5fr_buffer_test/"
            ...
            if len(all_branches) > 1:
                ...
                for branch in all_branches:
                    ...
                    for sub_label in branch:
                        ...
                        if (
                            (sub_end_t - sub_start_t) > traj_len_thres
                            and sub_start_t < begin_range
                            and sub_end_t > end_range
                        ):
                            ...
                            long_traj[max(0, sub_start_ind) : min(time_span - 1, sub_end_ind) + 1] = sub_traj[
                                max(0, sub_start_ind) : min(time_span - 1, sub_end_ind) + 1
                            ]
                            ...
                            long_traj_loss_ratio = (
                                whole_branch[whole_branch == -1].shape[0] * 1.0 / whole_branch.shape[0]
                            )
                            ...
                            if branch_end_t == time_span:
                                with open(
                                    sct_path + "sub_sct_" + posi_label + "_" + traj_name + "long_traj", "wb"
                                ) as fp:
