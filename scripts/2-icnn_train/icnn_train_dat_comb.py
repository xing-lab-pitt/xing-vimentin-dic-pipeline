...
if train_mode == 'am':
    ...
    train_datagen = ImageDataGenerator(rotation_range=180,zoom_range=(1,0.95), horizontal_flip=True,vertical_flip=True,validation_split=0.2)
elif train_mode == 'seg':
    train_data,train_label=prep_icnn_seg_train_data(train_path,obj_h,obj_w)
    train_datagen = ImageDataGenerator(rotation_range=180,zoom_range=(1,1.25), horizontal_flip=True,vertical_flip=True,validation_split=0.2)
...
batch_size = 150
patience = 45
early_stopping = EarlyStopping(monitor='val_loss', min_delta = 0.001, patience = patience, mode = 'auto', restore_best_weights = True)
...
validation_generator = train_datagen.flow(
    train_data,train_label, # same directory as training data
    batch_size=batch_size,
