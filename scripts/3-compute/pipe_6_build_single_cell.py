...
def build_single_cell(output_path):
    ...
    cells = [single_cell(img_num=df.loc[i, 'ImageNumber'],
                         obj_num=df.loc[i, 'ObjectNumber']) for i in range(len(df))]
    for i in range(len(cells)):
        ...
        # set_traj_label
        cells[i].set_traj_label(np.asscalar(df.loc[(df['ImageNumber'] == img_num) & (
            df['ObjectNumber'] == obj_num), 'Cell_TrackObjects_Label'].values))
        # set_apoptosis_mitosis_flag
        if ((am_record['ImageNumber'] == img_num) & (
                am_record['ObjectNumber'] == obj_num)).any():
            cells[i].set_am_flag(am_record.loc[(am_record['ImageNumber'] == img_num) & (
