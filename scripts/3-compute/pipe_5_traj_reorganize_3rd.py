...
def traj_reconganize3(output_path):
    ...
    if len(daughter_cells) > 0:
        ts_mask = np.all(np.any((traj_start - daughter_cells[:, None]), axis=2), axis=0)

    else:
        ts_mask = np.ones((traj_start.shape[0],), dtype=bool)

    if len(mother_cells) > 0:
        te_mask = np.all(np.any((traj_end - mother_cells[:, None]), axis=2), axis=0)
    else:
    ...
    a, gap_scores = get_gap_pair_scores(F, L, max_frame_difference)  # a is index pairs of L and F
    ...
    if np.sum(mask) > 0:
        ...
        score_matrix = scipy.sparse.coo_matrix((c, (i, j)), shape=(L.shape[0], F.shape[0])).tocsr()
    ...
    traj_seri_self.to_csv(dir_path + "traj_object_num" + ".csv", index=False, encoding="utf-8")
    ...
    mitosis_record.to_csv(dir_path + "mitosis_record" + ".csv", index=False, encoding="utf-8")
