...
#     print(np.amin(morph_sma[sort_morph_std_ind,0]))
    ...
    morph_scale_mr=cal_sma(sct.traj_feature[mask][:,key_mask],window_size=wd_size)[morph_scale_t,9]#median redius
...
#     print(SP)
    if SP.shape[0]>0 and np.amin(SP)<t_range:
        ...
        if SP.shape[0]>0:
            plt.scatter(norm_traj_morph[SP,0],norm_traj_vim[SP,0])
            plt.show()
        scale_contour,scale_contour_with_vim,scale_haralick,scale_t=np.array([]),np.array([]),np.array([]),np.array([])
...
def ssp_traj_scaling(sct,t_cutoff=6,t_range=48,Metric_L=1,norm_flag=False):
    ...
    morph_dist_cutoff=max(np.mean(np.linalg.norm(np.diff(traj_morph[:t_range],axis=0),axis=1,ord=Metric_L)),np.mean(np.linalg.norm(np.diff(traj_morph,axis=0),axis=1,ord=Metric_L)))
    vim_dist_cutoff=max(np.mean(np.linalg.norm(np.diff(traj_vim[:t_range],axis=0),axis=1,ord=Metric_L)),np.mean(np.linalg.norm(np.diff(traj_vim,axis=0),axis=1,ord=Metric_L)))
#     dist_cutoff=np.mean(np.linalg.norm(np.diff(X,axis=0),axis=1))
